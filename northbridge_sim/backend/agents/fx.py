...
class FXAgent(BaseAgent):
    """FX PM: focuses on FX symbols (Yahoo '=X' tickers)."""
    ...
    async def step(self) -> None:
        ...
        fx = [x for x in ins if x.get("asset_class") in ("fx", "forex") or str(x.get("symbol","")).upper().endswith("=X")]
        if not fx:
            await self.log_state({"state": "idle", "reason": "no FX instruments in universe"})
            return
        ...
        for inst in fx:
            ...
            if len(prices) < 10:
                continue
            mom, vol = simple_momentum_signal(prices, lookback=90)
            score = mom / (vol + 1e-6)
            scored.append((abs(score), inst, score, mom, vol))

        if not scored:
            await self.log_state({"state": "waiting_data", "tracked": len(fx)})
            return

        scored.sort(key=lambda x: x[0], reverse=True)
        _, inst, score, mom, vol = scored[0]
        sym = inst["symbol"]
        venue = inst.get("meta", {}).get("preferred_venue") or "YAHOO"
        ...
        if signals_store:
            try:
                sigs = await signals_store.recent(limit=5, category="fx")
            except Exception:
                sigs = []

        thesis = (
            f"FX momentum/risk-adjusted score={score:.3f} (mom={mom:.3%}, vol={vol:.4f}) on {sym}.\n"
        ...
        intent = TradeIntent(
            ...
            thesis=thesis,
            risk_notes="FX priced via Yahoo; sizing ~2% NAV notional; CRO will enforce firm limits.",
            tags=["fx", "momentum"],
        )
